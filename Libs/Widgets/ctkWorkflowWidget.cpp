                                       ...
                                       ctkWorkflowWidgetStep* step)
{
  ...
  if (item == "[<-]")
    ...
    {
    res = QVariant(step ? step->workflow()->backwardSteps(step).count() + 1 : 0);
    }
  else if (item == "{!#}" || item == "(!#)")
    {
               ...
               ctkWorkflowWidgetStep* step)
{
  QMap<QString, QVariant> formats =
    ctkWorkflowWidget::format(buttonFormat, step);
...
{
  QMap<QString, QVariant> formats =
    ctkWorkflowWidget::format(textFormat, step);
  return formats["text"].toString();
}

//-----------------------------------------------------------------------------
QMap<QString, QVariant> ctkWorkflowWidget
::format(const QString& format, ctkWorkflowWidgetStep* step)
{
  QIcon buttonIcon;
